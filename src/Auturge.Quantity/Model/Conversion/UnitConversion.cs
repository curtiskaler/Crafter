...
public class UnitConversion(Unit sourceUnit, Unit targetUnit) : IUnitConversion, IEquatable<UnitConversion> 
{
    public Unit SourceUnit { get; } = sourceUnit;
    public Unit TargetUnit { get; } = targetUnit;
    public Conversion Conversion { get; } = Conversion.Identity;

    public UnitConversion(Unit sourceUnit, Unit targetUnit, Conversion conversion) : this(sourceUnit, targetUnit)
    {
    ...
    internal UnitConversion(Unit sourceUnit, Unit targetUnit,
        ...
        Func<object, object> inversion)
        : this(sourceUnit, targetUnit, new Conversion(conversion, inversion))
    {
    }

    internal UnitConversion(List<UnitConversion> list) : this(list.First().SourceUnit, list.Last().TargetUnit)
    {
    ...
    public static UnitConversion operator *(UnitConversion lhs, UnitConversion rhs)
    {
        // this means: do conversion 1, then do conversion 2
        return new UnitConversion(lhs.SourceUnit, rhs.TargetUnit, lhs.Conversion * rhs.Conversion);
    }
    ...
    public static implicit operator UnitConversion(List<UnitConversion> list) => new(list);

    public UnitConversion Invert() => Invert(this);

    public static UnitConversion Invert(UnitConversion toInvert)
        => new(toInvert.TargetUnit, toInvert.SourceUnit, toInvert.Conversion.Invert());


    public static UnitConversion Create(Unit source, Unit target, Func<object, object> conversion,
        Func<object, object> inversion) =>
    ...
    public override bool Equals(object? obj) => Equals(obj as UnitConversion);
